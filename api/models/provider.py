...
class Provider(Base):
    ...
    provider_type: Mapped[str] = mapped_column(
        String(40), nullable=False, **varchar_default("custom")
    )
    is_valid: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, server_default=text("false"))
    last_used: Mapped[Optional[datetime]] = mapped_column(sa.DateTime, nullable=True)
    credential_id: Mapped[Optional[str]] = mapped_column(StringUUID, nullable=True)

    quota_type: Mapped[Optional[str]] = mapped_column(String(40), nullable=True, **varchar_default(""))
...
class ProviderOrder(Base):
    ...
    payment_status: Mapped[str] = mapped_column(
        String(40), nullable=False, **varchar_default("wait_pay")
    )
...
class ProviderCredential(Base):
    ...
    # NOTE (shiver): We use uuid_v4 here, though origin implementation has turned to uuidv7()
    id: Mapped[str] = mapped_column(StringUUID, **uuid_default())
    tenant_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
    provider_name: Mapped[str] = mapped_column(String(255), nullable=False)
