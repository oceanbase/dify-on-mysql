...
class MysqlRedisClient:
    ...
    def _start_cleanup_thread(self):
        ...
        if not self.cleanup_thread_is_alive():
            ...
            self._cleanup_thread = threading.Thread(
                target=self._cleanup_expired_cache, daemon=True, name="MysqlRedisClient-Cleanup"
            )
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            expired_count = (
                self.db.session.query(Cache)
                .filter(Cache.expire_time.isnot(None), Cache.expire_time < datetime.now())
                .delete()
            )
            ...
            if "1146" in err_str and "doesn't exist" in err_str:
                return 0
            logger.warning(f"Error during manual cache cleanup: {err_str}")
            try:
    ...
    def get(self, name: str) -> Optional[bytes]:
        ...
        try:
            cache_item = (
                self.db.session.query(Cache)
                .filter(Cache.cache_key == name, or_(Cache.expire_time.is_(None), Cache.expire_time > datetime.now()))
                .first()
            )
    ...
    def set(self, name: str, value, ex: None | int | timedelta = None) -> None:
        ...
        try:
            ...
            """
            self.db.session.execute(db.text(sql), {"cache_key": name, "cache_value": value, "expire_time": expire_time})
            self.db.session.commit()
        except Exception as e:
    ...
    def setnx(self, name: str, value) -> None:
        ...
        try:
            ...
            VALUES (:cache_key, :cache_value, :expire_time)
            """
            self.db.session.execute(db.text(sql), {"cache_key": name, "cache_value": value, "expire_time": None})
            self.db.session.commit()
        except Exception as e:
    ...
    def expire(self, name: str, time: int | timedelta) -> None:
        ...
        try:
            ...
            """
            result = self.db.session.execute(db.text(sql), {"cache_key": name, "expire_time": expire_time})
            self.db.session.commit()
        except Exception as e:
...
class MysqlLock:
    ...
    def acquire(self, blocking: bool = True) -> bool:
        ...
        assert self.timeout is not None, "timeout must be set when acquiring a lock in blocking mode"
        import time
    ...
    def _try_acquire(self) -> bool:
        ...
        try:
            ...
            lock_value = {"process_id": process_id, "thread_id": thread_id, "timestamp": current_time}
        ...
        try:
            ...
            if cache_item:
                try:
                    ...
                    if lock_data.get("process_id") == process_id and lock_data.get("thread_id") == thread_id:
...
def main():
    ...
    with app.app_context():
        ...
        try:
            ...
            # Test 10: Lock blocking functionality
            print("Test 10: Lock blocking functionality")
            lock_name = "test_lock"
            # non blocking lock
            lock = client.lock(lock_name)
            assert lock.acquire(blocking=False) == True
            new_lock = client.lock(lock_name)
            assert new_lock.acquire(blocking=False) == False
            lock.release()
            assert new_lock.acquire(blocking=False) == True
            new_lock.release()
            # blocking lock
            lock = client.lock(lock_name, timeout=20)
            assert lock.acquire(blocking=True) == True
            new_lock = client.lock(lock_name, timeout=1)
            assert new_lock.acquire(blocking=True) == False
            lock.release()
            assert new_lock.acquire(blocking=True) == True
            new_lock.release()
            print("✓ Lock blocking functionality test passed")
